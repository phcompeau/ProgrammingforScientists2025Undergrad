...
def prime_count_array(n: int):
    ...
    """
    We will return a list of how many primes we've seen so far at a given index. 
    input: n -> int
    output: list of how many primes we've seen so far at a given index.
    """
    prime_list = sieve_of_eratosthenes(n)
    result = [0] * (n + 1)
    counter = 0
    for i in range(len(prime_list)):
        is_prime = prime_list[i]
        if is_prime:
            counter = counter + 1
        result[i] = counter
    return result
